...
- Preserve the intervals.icu activity type in each exported activity for future multi-sport support
...
├── contracts/
│   └── week-data/
│       ├── week-data.schema.json   # JSON Schema for consolidated week_data payload
│       ├── WeekDataDto.cs          # C# DTOs for consolidated week_data payload (System.Text.Json)
...
│       ├── training_readiness.py   # Shared readiness traffic-light calculation
│       ├── week_data_schema.py     # Pydantic models for consolidated week_data JSON schema
...
### `generate_week_data_schema.py`

Generates contract artifacts for the consolidated `week_data` / `coach_input` payload returned by `prepare_week_data`.
By default it writes both the JSON Schema and the C# DTO classes.

```bash
python scripts/generate_week_data_schema.py
python scripts/generate_week_data_schema.py --schema-output ./contracts/week-data/week-data.schema.json
python scripts/generate_week_data_schema.py --dto-output ./contracts/week-data/WeekDataDto.cs
```

Output:
- `contracts/week-data/week-data.schema.json`
- `contracts/week-data/WeekDataDto.cs`
...
C# DTO set (System.Text.Json) for the consolidated `week_data` payload generated by `prepare_week_data` / `coach_input`.
The classes use `JsonPropertyName` for snake_case mapping and `JsonExtensionData` for forward-compatible extra fields.
The file is generated by `scripts/generate_week_data_schema.py`.

### `contracts/week-data/WeekDataJson.cs`

C# utility helpers for deserialization/serialization with safe default `JsonSerializerOptions` for `WeekDataDto`.

### `contracts/week-data/week-data.schema.json`

Generated JSON Schema artifact for the same `week_data` contract, intended for CI/build consumption and cross-language validation.
