...
| Tool | Description |
|---|---|
| `prepare_week_data` | Runs the full data pipeline (activities, metrics, training plan, fueling, week analysis) and returns the consolidated coach input as JSON, including the daily traffic light under `week_summary.training_readiness`. Supports optional `lookback_days` (default `7`) for activity/fueling windows while keeping `week_summary` anchored to calendar week. Nothing is stored on the server. |
...
| `OAUTH_CLIENT_TABLE_NAME` | `mcpoauthclients` | Azure Table name used for OAuth dynamic client registrations. |

OAuth client registrations are cleaned up by the service after 200 days. Azure Table Storage does not provide a native TTL policy for these entities, so the app runs a periodic retention cleanup and also removes expired entries on write.
...
└── infra/
    ...
    └── modules/
      ...
      └── oauth-client-registry.bicep  # Existing Storage Account table + RBAC for OAuth client registrations
```
